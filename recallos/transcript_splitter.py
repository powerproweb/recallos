#!/usr/bin/env python3
"""
transcript_splitter.py â€” Split concatenated transcript files into per-session files
=================================================================================
...
Usage:
    python3 transcript_splitter.py                          # scan ~/Desktop/transcripts
    python3 transcript_splitter.py --source ~/Desktop/transcripts  # explicit source
    python3 transcript_splitter.py --dry-run                # show what would happen
    python3 transcript_splitter.py --min-sessions 2         # only files with 2+ sessions
...
HOME = Path.home()
LUMI_DIR = Path(os.environ.get("RECALLOS_SOURCE_DIR", str(HOME / "Desktop/transcripts")))

# People we know about (for name detection in content)
# Loaded from ~/.recallos/known_names.json if it exists, otherwise generic fallback.
_KNOWN_NAMES_PATH = HOME / ".recallos" / "known_names.json"
...
def _load_known_people() -> list:
    ...
    if _KNOWN_NAMES_PATH.exists():
        try:
            ...
            pass
    # Generic fallback â€” override by creating ~/.recallos/known_names.json
...
def extract_timestamp(lines):
    """
    Find the first timestamp line: âº H:MM AM/PM Weekday, Month DD, YYYY
    Returns (datetime_str, iso_str) or (None, None).
    """
    ts_pattern = re.compile(r"âº\s+(\d{1,2}:\d{2}\s+[AP]M)\s+\w+,\s+(\w+)\s+(\d{1,2}),\s+(\d{4})")
    months = {
        "January": "01",
        "February": "02",
        "March": "03",
...
def extract_people(lines):
    ...
    # Working directory username hint â€” map to known people if configured
    dir_match = re.search(r"/Users/(\w+)/", text)
    if dir_match:
        username = dir_match.group(1)
        # User can map usernames to names in ~/.recallos/known_names.json
...
def split_file(filepath, output_dir, dry_run=False):
    ...
    for i, (start, end) in enumerate(zip(boundaries, boundaries[1:])):
        ...
        if dry_run:
            ...
            print(f"  âœ“ {name}  ({len(chunk)} lines)")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Source directory (default: RECALLOS_SOURCE_DIR or ~/Desktop/transcripts)",
    )
    parser.add_argument(
    ...
    print(f"\n{'=' * 60}")
    print(f"  Mega-file splitter â€” {'DRY RUN' if args.dry_run else 'SPLITTING'}")
    print(f"{'=' * 60}")
    ...
    for f, n_sessions in mega_files:
        ...
        if not args.dry_run and written:
            ...
            f.rename(backup)
            print(f"  â†’ Original renamed to {backup.name}\n")
        else:
            print()

    print(f"{'â”€' * 60}")
    if args.dry_run:
        print(f"  DRY RUN â€” would create {total_written} files from {len(mega_files)} mega-files")
    else:
        print(f"  Done â€” created {total_written} files from {len(mega_files)} mega-files")
    print()
