#!/usr/bin/env python3
"""
entity_detector.py â€” Auto-detect people and projects from file content.
...
Used by recallos init before mining begins.
...
# Person signals â€” things people do
...
# Person signals â€” pronouns resolving nearby
PRONOUN_PATTERNS = [
    r"\bshe\b",
    r"\bher\b",
    r"\bhers\b",
...
# Person signals â€” dialogue markers
DIALOGUE_PATTERNS = [
    r"^>\s*{name}[:\s]",  # > Speaker: ...
    r"^{name}:\s",  # Speaker: ...
    r"^\[{name}\]",  # [Speaker]
...
# Project signals â€” things projects have/do
...
# For entity detection â€” prose only, no code files
...
def extract_candidates(text: str) -> dict:
    ...
    """
    # Find all capitalized words (not at sentence start â€” harder, so we use frequency as filter)
    ...
    # Also find multi-word proper nouns (e.g. "Memory vault", "Claude Code")
...
def score_entity(name: str, text: str, lines: list) -> dict:
    ...
    # Pronoun proximity â€” pronouns within 3 lines of the name
...
def classify_entity(name: str, frequency: int, scores: dict) -> dict:
    ...
    if total == 0:
        # No strong signals â€” frequency-only candidate, uncertain
    ...
    # One signal type with many hits (e.g. "Click, click, click...") is not enough â€”
    ...
    if person_ratio >= 0.7 and has_two_signal_types and ps >= 5:
        ...
        # Pronoun-only match â€” downgrade to uncertain
        entity_type = "uncertain"
        confidence = 0.4
        signals = scores["person_signals"] + [f"appears {frequency}x â€” pronoun-only match"]
        ...
        signals.append("mixed signals â€” needs review")
...
def detect_entities(file_paths: list, max_files: int = 10) -> dict:
    ...
    MAX_BYTES_PER_FILE = 5_000  # first 5KB per file â€” enough to catch recurring entities
...
def _print_entity_list(entities: list, label: str):
    ...
    for i, e in enumerate(entities):
        confidence_bar = "â—" * int(e["confidence"] * 5) + "â—‹" * (5 - int(e["confidence"] * 5))
...
def confirm_entities(detected: dict, yes: bool = False) -> dict:
    ...
    """
    print(f"\n{'=' * 58}")
    print("  recallos â€” Entity Detection")
    print(f"{'=' * 58}")
    ...
    if yes:
        # Auto-accept: include all detected (skip uncertain â€” ambiguous without user input)
        print(
    ...
    if choice == "edit":
        ...
        if detected["uncertain"]:
            print("\n  Uncertain entities â€” classify each:")
            for e in detected["uncertain"]:
                ans = input(f"    {e['name']} â€” (p)erson, (r)roject, or (s)kip? ").strip().lower()
                if ans == "p":
...
def scan_for_detection(project_dir: str, max_files: int = 10) -> list:
    ...
    Prose only (.txt, .md, .rst, .csv) â€” code files produce too many false positives.
    ...
    # Prefer prose files â€” fall back to all readable if too few prose files
