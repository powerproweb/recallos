#!/usr/bin/env python3
"""
entity_registry.py â€” Persistent personal entity registry for recallos.

Knows the difference between Riley (a person) and ever (an adverb).
Built from three sources, in priority order:
  1. Onboarding â€” what the user explicitly told us
  2. Learned â€” what we inferred from session history with high confidence
  3. Researched â€” what we looked up via Wikipedia for unknown words
...
Usage:
    ...
    # â†’ {"type": "person", "confidence": 1.0, "source": "onboarding"}
"""
...
def _wikipedia_lookup(word: str) -> dict:
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "recallos/1.0"})
        ...
        # Disambiguation â€” look at description
        ...
        if e.code == 404:
            # Not in Wikipedia â€” strong signal it's a proper noun (unusual name, nickname)
            return {
                "inferred_type": "person",
                "confidence": 0.70,
                "wiki_summary": None,
                "wiki_title": None,
                "note": "not found in Wikipedia â€” likely a proper noun or unusual name",
            }
...
class EntityRegistry:
    ...
    Stored at ~/.recallos/entity_registry.json
    Schema:
    {
    ...
    DEFAULT_PATH = Path.home() / ".recallos" / "entity_registry.json"
    ...
    # â”€â”€ Seed from onboarding â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def seed(self, mode: str, people: list, projects: list, aliases: dict = None):
        ...
        aliases = aliases or {}
        reverse_aliases = {v: k for k, v in aliases.items()}  # Maxwell â†’ Max
    ...
    def _disambiguate(self, word: str, context: str, person_info: dict) -> Optional[dict]:
        ...
        # Truly ambiguous â€” return None to fall through to person (registered name)
        return None

    # â”€â”€ Research unknown words â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Learn from sessions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def learn_from_text(self, text: str, min_confidence: float = 0.75) -> list:
        ...
        """
        from recallos.entity_detector import extract_candidates, score_entity, classify_entity
    ...
    # â”€â”€ Query helpers for retrieval â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
