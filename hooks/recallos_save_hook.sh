#!/bin/bash
# RECALLOS SAVE HOOK — Auto-save every N exchanges
#
...
# 3. Returns a reason telling the AI to save structured log + vault entries
# 4. AI does the save (topics, decisions, code, quotes → organized into vault)
# 5. Next Stop fires with stop_hook_active=true → lets AI stop normally
#
# The AI does the classification — it knows what domain/node to use
...
#       "hooks": [{
#         "type": "command",
#         "command": "/absolute/path/to/recallos_save_hook.sh",
#         "timeout": 30
#       }]
#     }]
#   }
...
#
# === RECALLOS CLI ===
# This repo uses: recallos ingest <dir>
# or:            recallos ingest <dir> --mode convos
# Set RECALLOS_DIR below if you want the hook to auto-ingest after blocking.
# Leave blank to rely on the AI's own save instructions.
#
# === CONFIGURATION ===

SAVE_INTERVAL=15  # Save every N human messages (adjust to taste)
STATE_DIR="$HOME/.recallos/hook_state"
mkdir -p "$STATE_DIR"

# Optional: set to the directory you want auto-ingested on each save trigger.
# Example: RECALLOS_DIR="$HOME/conversations"
...
# Log for debugging (check ~/.recallos/hook_state/hook.log)
...
if [ "$SINCE_LAST" -ge "$SAVE_INTERVAL" ] && [ "$EXCHANGE_COUNT" -gt 0 ]; then
    ...
    # Optional: run recallos ingest in background if RECALLOS_DIR is set
    if [ -n "$RECALLOS_DIR" ] && [ -d "$RECALLOS_DIR" ]; then
        SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
        REPO_DIR="$(dirname "$SCRIPT_DIR")"
        python3 -m recallos ingest "$RECALLOS_DIR" >> "$STATE_DIR/hook.log" 2>&1 &
    fi
